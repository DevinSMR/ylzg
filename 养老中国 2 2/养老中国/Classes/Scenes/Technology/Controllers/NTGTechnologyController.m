...
@interface NTGTechnologyController ()<UITableViewDelegate,UITableViewDataSource,NJBannerViewDataSource,UISearchBarDelegate>
...
@implementation NTGTechnologyController
...
-(void)requestCycleViewData{
    ...
    if ([NTGDetection netStatus] == NotReachable) {
        ...
        [NTGSendRequest getTechnologyBanner:nil success:result2];
    }
...
- (void) initData :(NTGPullRefresh)action{
    ...
    if ([NTGDetection netStatus] == NotReachable) {
        ...
        [self addHudWithMessage:@"加载中..."];
        [NTGSendRequest getListTechnology:self.reqParam success:result];
    }
