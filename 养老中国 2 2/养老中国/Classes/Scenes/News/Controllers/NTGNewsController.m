...
@interface NTGNewsController ()<UITableViewDelegate,UITableViewDataSource,NJBannerViewDataSource,UISearchBarDelegate>
...
@implementation NTGNewsController
...
-(void)requestCycleViewData{
    ...
    if ([NTGDetection netStatus] == NotReachable) {
        ...
        [NTGSendRequest getNewsBanner:self.reqParam success:result2];
    }
...
- (void) initData :(NTGPullRefresh)action{
    ...
    if ([NTGDetection netStatus] == NotReachable) {
        ...
        [self addHudWithMessage:@"加载中..."];
        [NTGSendRequest getListNews:self.reqParam success:result];
    }
